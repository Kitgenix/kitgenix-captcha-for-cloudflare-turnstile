...
(function () {
  ...
  /* ─────────────────────────────────────
     ...
     A table can combine a free-text search input (data-kitgenix-table-search)
     with any number of structured dropdown filters (data-kitgenix-table-filter
     ="fieldname", matching a data-fieldname attribute on each row). Both kinds
     of control resolve to the same table container and share one visibility
     state on it (container.kitgenixFilterState), so e.g. typing in the search
     box and picking "Failed" from an Outcome dropdown narrow the same rows
     together rather than fighting over data-kitgenix-search-hidden.
  ───────────────────────────────────── */

  function getFilterTargetContainer(el) {
    var targetSelector = el.getAttribute('data-kitgenix-table-search-target') || el.getAttribute('data-kitgenix-table-filter-target');
    var container = null;
    if (targetSelector) {
      try { container = document.querySelector(targetSelector); } catch (_e) {}
    }
    if (!container) {
      container = (el.closest && el.closest('.kitgenix-table-wrap')) || null;
    }
    return container;
  }

  function timeFilterCutoff(value) {
    var now = Math.floor(Date.now() / 1000);
    switch (value) {
      case '1h': return now - 3600;
      case '24h': return now - 86400;
      case '7d': return now - (86400 * 7);
      case '30d': return now - (86400 * 30);
      default: return 0;
    }
  }

  function recomputeTableVisibility(container) {
    try {
      if (!container) return;
      ...
      var state = container.kitgenixFilterState || { query: '', filters: {} };
      var query = (state.query || '').trim().toLowerCase();
      var filters = state.filters || {};
      var anyFilterActive = !!query;
      for (var fk in filters) {
        if (Object.prototype.hasOwnProperty.call(filters, fk) && filters[fk]) {
      ...
      rows.forEach(function (row) {
        ...
        if (match) {
          for (var key in filters) {
            ...
            var val = filters[key];
            if (!val || key === 'time') continue;
            if (row.getAttribute('data-' + key) !== val) {
              match = false;
              break;
            }
          }
        ...
        if (match && filters.time) {
          var cutoff = timeFilterCutoff(filters.time);
          if (cutoff) {
            var ts = parseInt(row.getAttribute('data-time') || '0', 10);
            if (!ts || ts < cutoff) match = false;
          }
        }
      ...
      if (emptyEl) {
        emptyEl.style.display = (anyFilterActive && visible === 0) ? '' : 'none';
      }
  ...
  function bindTableSearch(input) {
    try {
      ...
      var container = getFilterTargetContainer(input);
      if (!container) return;
      container.kitgenixFilterState = container.kitgenixFilterState || { query: '', filters: {} };

      function run() {
        container.kitgenixFilterState.query = input.value || '';
        recomputeTableVisibility(container);
      }
  ...
  function bindTableFilterSelect(select) {
    try {
      if (!select || select.getAttribute('data-kitgenix-bound') === '1') return;
      select.setAttribute('data-kitgenix-bound', '1');

      var field = select.getAttribute('data-kitgenix-table-filter');
      if (!field) return;

      var container = getFilterTargetContainer(select);
      if (!container) return;
      container.kitgenixFilterState = container.kitgenixFilterState || { query: '', filters: {} };

      select.addEventListener('change', function () {
        container.kitgenixFilterState.filters[field] = select.value || '';
        recomputeTableVisibility(container);
      });
    } catch (_e) {}
  }
  ...
  function initTableFilters(root) {
    try {
      var scope = root || document;
      var selects = toArray(scope.querySelectorAll('[data-kitgenix-table-filter]'));
      selects.forEach(bindTableFilterSelect);
    } catch (_e) {}
  }
